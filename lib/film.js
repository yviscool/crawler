...
module.exports.getLatest = function () {
    ...
    var getLatestFilm = () => painyuan.getLatestFilm();

    var select = function (msg) {
        return onFulfilled(msg);
    ...
    var getDownLoadLinkORSearch = function (msg) {
        return onFulfilled(msg);
        function onFulfilled(msg) {
            if (typeof msg === 'object') {
                return painyuan.getDownLoadLink(msg);
            } else if (msg === 'search') {
                return RL.questionPromise(Prompt.filmInfo)
                    .then(select)
