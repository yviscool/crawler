...
module.exports.search = function () {
    ...
    RL.questionPromise(Prompt.input)
        ...
        (e) => {
            if (e.message === 'ETIMEDOUT' || e.message === 'ESOCKETTIMEDOUT') {
                return biquge.searchByName(RL.currentKey)
        ...
        (e) => { if (e.message.length < 10) { return RL.questionPromise(Prompt.download) }; throw new Error(e.message) })
        .then((input) => {
            input = input === '' ?
                'default' : /^\d$/.test(input) ?
                    parseInt(input) : input === '10' ?
                        'back' : function () { throw new Error('输入不合法，请重新输入, ctrl + u 清除该信息') }();
            return input;
        })
        .then((key) => {
        ...
        .then(([chapterUrl,novelName,source])=>{Novel.download(chapterUrl,novelName,source)})
        .catch(begin)
