...
module.exports.getMusic = function () {
    ...
    RL.question(PROMPTS.searchMusicOrgetAudio)
        ...
        return RL.questionPromise(Prompt.getMusic)
            .then(key => {
                if (/^\d{1,2}$/.test(key) && parseInt(key) <= audiojungleMsg.category.length - 1) {
                    ...
                    downloadMusicByAudioJungle(config.options)
                }
                throw new CustomError.InputError();
            })
...
class Music {
    ...
    download(name, mp3Url) {
        ...
        this.RL.prompt()
        this.RL.write('正在异步写入，完成之后会自动播放音乐, ctrl + u 清除该信息')
